...
@Component(
	...
	property = {
		...
		"javax.portlet.init-param.view-template=/viewUser.jsp",
	...
	public void addUser(ActionRequest request, ActionResponse response) {
		...
		try {
			...
			if (Validator.isNotNull(addUser)) {
				...
				leaveTracker.setUserId(tDisplay.getUserId());
				
				leaveTracker.setCreateDate(new Date());
				leaveTracker.setModifiedDate(new Date());
				leaveTracker.setCreatedBy(tDisplay.getUserId());
				leaveTracker.setModifiedBy(tDisplay.getUserId());
